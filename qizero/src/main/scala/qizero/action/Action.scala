...
import qizero.failure.{Failure, NotImplementedFailure}
...
object Action {
  ...
  def apply[R](f: () => R): Action[R] = new Action[R] {
    protected def act(): R = f()
  }

  implicit final class Runner[R](action: Action[R]) {
    def run: R = action.invoke()
    def future(implicit executor: ExecutionContext = global): Future[R] = Future(run)
  }
